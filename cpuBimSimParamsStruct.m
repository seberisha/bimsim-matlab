function params = cpuBimSimParamsStruct(input)
...
ordVecEf=((0:input.orderEf)'); %the prefix term (2l + 1)*i^l
...
if input.fieldInsideSphere
    y = zeros(params.simRes,params.simRes);
else
...
params.Pl_cosalpha1 = cpuLegendre(input.orderEf+1,cos(params.alpha1));
params.Pl_cosalpha2 = cpuLegendre(input.orderEf+1,cos(params.alpha2));
...
%band pass filter
params.BPF = cpuBPF(halfGridSize, params.simRes, input.objectiveNA_in/params.lambda,  input.objectiveNA_out/params.lambda);
params.scale = 1;

%generate number of samples per ring

if input.constantNumSamples
    params.numPoints = ones(numel(params.rad))*round(params.resolution/4);
    %params.numPoints = ones(numel(params.rad))*(params.resolution/2);
    %params.numPoints = ones(size(params.rad)).*ceil(((2*pi.*params.rad(end)-params.spacing)./params.spacing + 1));
    params.theta = 2*pi*params.numPoints:2*pi*params.numPoints:2*pi;

else
    params.numPoints = ones(size(params.rad));
    params.numPoints(2:end) = ceil(((2*pi.*params.rad(2:end))));
end
