...
for i=1:numWav
    ...
    [pE_s, pE_i] = precomputeForExtendedSource(params);
    ...
    for p = 2:params.numPS
        ...
        %tempEf = newComputeEf(params);
        %subplot(1,2,1),imagesc(abs(tempEf)),axis image,colorbar,colormap(brewer)
        ...
        [E_s, E_i] = computeScatteredFields(params);
        E_t(params.psMask<params.a) = E_i(params.psMask<params.a);
        E_t(params.psMask>=params.a) = tempEf(params.psMask>=params.a) + E_s(params.psMask>=params.a);
        
        E_t(isnan(E_t)) = 0;    E_t(isinf(E_t))=0;%near field = out_n in cuda bimsim
        %figure
        %subplot(1,2,1),imagesc(abs(E_t)),axis image,colorbar,colormap(brewer)
        
        [E_s, E_i] = extendedSourceScatteredFields(params, pE_s, pE_i);
        E_t = zeros(size(E_s));
        E_t(params.psMask<params.a) = E_i(params.psMask<params.a);
        E_t(params.psMask>=params.a) = tempEf(params.psMask>=params.a) + E_s(params.psMask>=params.a);
        E_t(isnan(E_t)) = 0;    E_t(isinf(E_t))=0;%near field = out_n in cuda bimsim
        
        %subplot(1,2,2),imagesc(abs(E_t)),axis image,colorbar,colormap(brewer)
