...
fraction = 3;
points = 10;
[pf_x, pf_z] = pointSources(samples, fraction, points);
...
for i=1:numWav
    ...
    for p = 2:params.numPS
        ...
        %subplot(1,2,1),imagesc(abs(E_fi)),axis image ,colorbar,colormap(brewer)
        ...
        %% compute Et using precomputed Es,Ei
        [E_s, E_i] = extendedSourceScatteredFields(params, pE_s, pE_i);
        E_t = zeros(size(E_s));
        E_t(params.psMask<params.a) = E_i(params.psMask<params.a);
        E_t(params.psMask>=params.a) = E_fi(params.psMask>=params.a) + E_s(params.psMask>=params.a);
        E_t(isnan(E_t)) = 0;    E_t(isinf(E_t))=0;%near field = out_n in cuda bimsim
