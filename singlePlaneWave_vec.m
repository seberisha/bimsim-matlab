...
jp = j_kr.*P_ct_ef;
for order=0:N_l
    jp(:,order+1) = (2*order+1)*1i.^order*jp(:,order+1);
end
E_f = sum(jp,2);
...
for i=1:NW
    ...
    E_s = E_s + (1/NW)*computeEs_vec(B, h_kr, P_ct, N_l, 1);
    
    %Compute E_i
    E_i = E_i + (1/NW)*computeEi_vec(A, j_knr, P_ct, 1, N_l);
