...
n=1.4;
k_vec = [1 0 0];
%k_vec = monteCarlo(1,k_vec, NA_in, NA_out)';
...
%r=ones(res,res);
% theta = meshgrid(linspace(-pi,pi,res),linspace(-pi,pi,res));
...
for i=1:samples
   ...
   % E_f = E_f + subA.*exp(1i.*cos_theta);
    
    E_f = E_f + subA.*mcEf(j_kr, P_ct, order);
