function [D_Et, D_Ef] = computePointSources(startIdx, endIdx, BPF, E_t, i,params,pf_x,pf_z,pf_theta, D_Et,D_Ef,kVec,wavNum, simRes, origRVecs, Pl_cosalpha1, Pl_cosalpha2,il,r_ps,subA, k_j, normPMinPs, A,B, psMask)
...
for p=2:params.numPS
    ...
    %interpolate E_f
    %E_f = gpuComputeEf(params, kVec, wavNum,simRes,pf,origRVecs,Pl_cosalpha1,Pl_cosalpha2,il);
    E_f = imrotate(single(Ef_ps1), -pf_theta(p)*180/pi,'crop','bilinear');
    ...
    %subplot(221),imagesc(abs(orig_E_f)),axis image,colorbar,title('orig Ef')
    %subplot(222),imagesc(abs(E_f)),axis image,colorbar,title('interp Ef')
...
%     E_t(psMask>=params.a) = E_f(psMask>=params.a) + E_s(psMask>=params.a);
    ...
    E_t = imrotate(single(Et_ps1), -pf_theta(p)*180/pi,'crop','bilinear');
    
    %E_t = interp2(X, Y, (Et_ps1), Xii, Yii);
    E_t(isnan(E_t))=0; E_t(isinf(E_t))=0;
    
    %             subplot(223),imagesc(abs(orig_E_t)),axis image,colorbar,title('orig Et')
    %             subplot(224),imagesc(abs(E_t)),axis image,colorbar,title('interp Et')
    %             pause(1e-7)
